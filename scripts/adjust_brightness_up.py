...
# 사용자로부터 밝기 증가값 입력 받기 (입력 없으면 10)
user_input = input("얼마나 밝기를 올리시겠습니까? (기본값 10): ")
increase_amount = int(user_input) if user_input.strip() != '' else 10
...
if current_brightness < 100:
    new_brightness = min(current_brightness + increase_amount, 100)
    sbc.set_brightness(new_brightness, display=0)
    print(f"새 밝기: {new_brightness}%")
else:
    print("이미 최대 밝기 상태입니다.")
